...
#     input_directory_ratio_list: list[float],
...
# machine_learning._infer_from_model_and_write_pascal_voc_xml(
#     model_directory=Path("/mnt/c/users/wbfw109/MyDrive/shared_resource/training_padding_50_angle_90_map_900_recall_72"),
#     image_to_be_inferred_directory=Path("/mnt/c/users/wbfw109/image_task_211019_AreaSample_no_exif_test_dataset_Copy")
# )

image_directory = Path(
    "/mnt/c/Users/wbfw109/task_training_unknown_list/task_training_unknown-2021_12_14_08_33_18/Annotations/211214_AreaSamples"
)
annotation_directory = Path(
    "/mnt/c/Users/wbfw109/task_training_unknown_list/task_training_unknown-2021_12_14_08_33_18/JPEGImages/211214_AreaSamples"
)

count = 0
for image, annotation in zip(image_directory.iterdir(), annotation_directory.iterdir()):
    count += 1
    if count % 2 == 0:
        image.unlink()
        annotation.unlink()
...
copied_files_list_directory: Path = Path("/mnt/c/users/wbfw109/copied_files_list")
copied_files_list_directory.mkdir(exist_ok=True)
copied_files_directory: Path = Path(
    "/mnt/c/users/wbfw109/copied_files_list/copied_files"
)
copied_files_directory.mkdir(exist_ok=True)
image_list: list[Path] = []
for image in itertools.chain.from_iterable(
    [
        iterable.get_file_list(image_set_directory, extensions=["jpg"])
        for image_set_directory in [
            # Path("/mnt/c/users/wbfw109/211214_AreaSamples"),
            Path(
                "/mnt/c/users/wbfw109/task_training_unknown_list_sorted_true/task_training_unknown-2021_12_14_08_33_18"
            ),
            Path(
                "/mnt/c/users/wbfw109/image_task_211019_AreaSample_no_exif_test_dataset"
            ),
        ]
    ]
):
...
machine_learning.remove_EXIF_of_image_and_save(
    output_parent_directory=copied_files_directory, image_list=image_list
)
...
machine_learning._infer_from_model_and_write_pascal_voc_xml(
    model_directory="/mnt/c/users/wbfw109/MyDrive/shared_resource/training_padding_50_angle_90_map_900_recall_72",
    image_to_be_inferred_directory=copied_files_list_directory,
)
...
# x = pandas.concat(
#     [
#         machine_learning.convert_pascal_voc_xml_to_df_and_get(
#             xml_directory=image_set_dirctory
#         )
#         for image_set_dirctory in Path("/mnt/c/users/wbfw109/image_tasks_sorted").iterdir()
#     ]
# )
x = machine_learning.convert_pascal_voc_xml_to_df_and_get(
    xml_directory=Path("/mnt/c/users/wbfw109/copied_files_list_inferred_annotations")
)
x.head()
x.groupby(by="class").size()
