...
for image in itertools.chain.from_iterable(
    [
        ...
        for image_set_directory in [
            ...
            # Path("/mnt/c/users/wbfw109/image_task_211019_AreaSample_no_exif"),
            # Path("/mnt/c/users/wbfw109/image_task_211019_AreaSample_no_exif_test_dataset"),
        ]
    ]
):
    image_list.append(image)
    # print(image)
    # shutil.copy2(src=f"{image}", dst=f"{copied_files_directory}")
...
machine_learning._infer_from_model_and_write_pascal_voc_xml(
    model_directory="/mnt/c/Users/wbfw109/MyDrive/shared_resource/training_padding_50_map916_recall_75",
...
x = pandas.concat(
    [
        ...
        for image_set_dirctory in Path(
            "/mnt/c/users/wbfw109/image_tasks_sorted"
        ).iterdir()
    ]
)
x.head()
