...
from wbfw109.libs.typing import RecursiveTuple, SingleLinkedList  # type: ignore
...
def convert_syntax_tree_to_svgling_style(syntax_tree: SyntaxTree, /) -> RecursiveTuple:
    ...
    Example usage:
    ```
    with Path("ref/computer_science_words_korean.json").open("r", encoding="UTF-8") as f:
        data: list[ExplicitSimpleSyntaxNode] = json.load(f)
    svgling.draw_tree(
        *convert_syntax_tree_to_svgling_style(
            convert_explicit_syntax_node_list_to_tree(data)
        )
    )
    ```
    , or
