"""📝 Before use, input <service_key> variable !!!
...
for notice_nums, method_name in [
    ...
    for notice_num in notice_nums:
        ...
        url: str = f"http://apis.data.go.kr/1230000/BidPublicInfoService04/{method_name}?numOfRows=1&pageNo=1&ServiceKey={service_key}&inqryDiv=2&bidNtceNo={notice_num}&type=json"
        with urllib.request.urlopen(url) as response:
            data = response.read().decode("utf-8")
            json_obj = json.loads(data)
        target_obj = json_obj["response"]["body"]["items"][0]
        ...
        additional_url: str = rf"http://apis.data.go.kr/1230000/BidPublicInfoService04/{GET_E_FILE_METHOD_NAME}?numOfRows=1&pageNo=1&ServiceKey={service_key}&inqryDiv=2&bidNtceNo={notice_num}&type=json"
        ...
        if "items" in additional_json_obj["response"]["body"]:
            target_additional_obj = additional_json_obj["response"]["body"]["items"]
            target_obj.update(*target_additional_obj)
            # If attachments exist, download
            for doc_info in target_additional_obj:
                doc_url: str = doc_info[f"eorderAtchFileUrl"]
...
one_data_frame.to_csv(working_dir / "one_data_frame.csv", index=False)
