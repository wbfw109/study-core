...
def solution_17679(m: int, n: int, board: list[str]) -> int:
    ...
    Implementation
        ...
        - <firstly_popped_j> is required to bring not popped elements to the first popped index of the stack
        - <next_start_j> is required to create next <i_and_start_j>.
    Debugging
    ...
    while can_continue:
        ...
        for i, start_j in i_and_start_j:
            ...
            if i == nm1:
                if next_pop_set:
                    stacks[i][firstly_popped_j[i] :] = [
                        stacks[i][j]
                        for j in range(firstly_popped_j[i] + 2, len(stacks[i]))
                        if j not in next_pop_set
                    ]
                break
            ...
            # if stacks[i-1] is popped, <pop_set> will be inherited from <next_pop_set>.
            pop_set = next_pop_set if firstly_popped_j[im1] < m else set()
            next_pop_set = set()
            e_to_be_moved: list[str] = []
            # set <end_j>
            end_j = min(len(stacks[i]), len(stacks[ip1])) - 1
            print(i, start_j, end_j)
            ...
            for j in range(start_j, end_j):
                ...
                if stacks[i][j] == stacks[ip1][j] == stacks[i][jp1] == stacks[ip1][jp1]:
                    ...
                    if is_not_found_same_block:
                        ...
                        if j < firstly_popped_j[i]:
                            if i == 0:
                                firstly_popped_j[i] = j
                            else:
                                firstly_popped_j[i] = firstly_popped_j[im1] = j

                # Statement to bring not popped elements to the first popped index of the stack.
                if j > firstly_popped_j[i] and j not in pop_set:
                    e_to_be_moved.append(stacks[i][j])
            # Statement to bring not popped elements to the first popped index of the stack.
            if pop_set:
                # if firstly_popped_j[imi]
                firstly_popped_j[ip1] = firstly_popped_j[i]
                can_continue = True
                ...
                # move not popped elements to forward of stack if is_greater_than_first_pop_j
                stacks[i][firstly_popped_j[i] :] = e_to_be_moved
        for x in stacks:
            print(x)
        ...
        i_and_start_j = (
            (i, x - 1) if x > 1 else (i, 0)
            for i, x in enumerate(firstly_popped_j)
            if x < m
        )
